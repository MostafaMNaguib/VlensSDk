...
fun getBase64(imageUri: Uri,mContext: Context): String {
    val imageStream = mContext.contentResolver.openInputStream(imageUri)
    val selectedImage = BitmapFactory.decodeStream(imageStream)

    val bitmap = rotateImage(selectedImage, imageStream!!)
    val path = imageUri.path
    val file: File = File(path.toString())
    val fileSize: Float = java.lang.String.valueOf(file.length() / 1024.0).toFloat()
    Log.e(tag, "getBase64:Size ==> ${fileSize / 1024.0} " )
    val byteArrayOutputStream = ByteArrayOutputStream()
    selectedImage.compress(Bitmap.CompressFormat.PNG, 80, byteArrayOutputStream)
    val byteArray = byteArrayOutputStream.toByteArray()
    Log.e(tag, "getBase64: ${byteArray.size / (1024.0 * 1024.0)}" )
...
fun fastBlur(imageUri: Uri, mContext: Context, scale: Float, radius: Int): Bitmap? {

    val imageBitmap = MediaStore.Images.Media.getBitmap(mContext.contentResolver, imageUri)


    var sentBitmap = imageBitmap
